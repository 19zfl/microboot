...
public class PDFAction {
    ...
    public void createPDFData(HttpServletResponse response) throws Exception { // 使用Response是为了响应处理
        ...
        // 一般情况下可以将生成的PDF文件进行下载，所以此时强制性的开启下载，并且配置下载文件名称
        response.setHeader("Content-Disposition", "attachement;filename=luo.pdf");
        ...
        System.out.println("A4纸高度="+PageSize.A4.getHeight());
        System.out.println("图片高度="+image.getHeight());
        image.setAbsolutePosition(pointX, pointY); // 绘制图片坐标的定义
        ...
        // 在PDF文件上开始绘制文本信息
        String titles[] = new String[] {"返回PDF数据", "Spring Boot", "数据响应"};
